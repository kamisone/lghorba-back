...
const StatusSchema = z.enum(['subscribed', 'unsubscribed', 'bounced'])
  .transform((v) => v as NewsletterSubscriberStatus);
...
export type CreateSubscriberAdminDto = z.infer<typeof CreateSubscriberAdminSchema>;
...
export const BulkSubscriberActionSchema = z.object({
  ...
  action: z.enum(['unsubscribe', 'resubscribe', 'delete', 'add_tag', 'remove_tag']),
  tag: z.string().max(50).optional(),
}).refine(
  (data) => (data.action !== 'add_tag' && data.action !== 'remove_tag') || !!data.tag,
  { message: 'tag is required for add_tag/remove_tag actions', path: ['tag'] },
);
