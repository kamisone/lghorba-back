...
import { CampaignStatus } from './newsletter-campaign.entity';
...
export class NewsletterCampaignsController {
  ...
  async audiencePreview(@Body(new ZodValidationPipe(AudiencePreviewSchema)) dto: AudiencePreviewDto) {
  ...
  @Post(':id/schedule')
  schedule(@Param('id') id: string, @Body(new ZodValidationPipe(ScheduleCampaignSchema)) dto: ScheduleCampaignDto) {
  ...
  @Post(':id/send-test')
  async sendTest(@Param('id') id: string, @Body(new ZodValidationPipe(SendTestEmailSchema)) dto: SendTestEmailDto) {
