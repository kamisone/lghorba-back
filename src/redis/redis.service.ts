...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    this._client = new Redis({
      ...
      // (EAI_AGAIN from the cluster resolver) would then leave this pod with a
      // permanently dead Redis client until it is restarted.
      retryStrategy: (times) => Math.min(times * 200, 5_000),

      // Bounds every command so an unreachable Redis degrades instead of
      // hanging. Callers already fall back on a rejected promise, but a catch
      // block never runs while a promise is simply pending: without this, a
      // single GET sat in the offline queue for ~92 s (16 reconnects × a 5 s
      // connect timeout) before rejecting, far past any ingress or browser
      // timeout. This is what made the site hang rather than serve uncached.
    ...
    // Retrying forever means an outage emits an error every few seconds. Log at
    // most one per interval so a Redis outage cannot bury every other log line.
