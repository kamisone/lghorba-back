...
/**
 * fr + the 5 non-English overlay languages — everything the shop needs
 * translated once an admin has written a section in English, which is the
 * source language for every "Generate" button on the product-edit page.
 */
...
/**
 * Powers every "Generate" button across the shop admin (product title, short
 * description, description, specifications, FAQs, story gallery, trust
 * badges, social videos, and variant attribute names/option display values):
 * the admin writes the English source, clicks Generate, and gets French (the
 * shop's base language) plus the 5 other overlay languages back — via free
 * Google Translate (see FreeTranslateService).
 */
@Injectable()
export class TranslationService {
  ...
  /** Translates a variant attribute's name (e.g. "Color", "Size"). */
  ...
  /** Translates a variant option's display value (e.g. "Red", "Large"). */
