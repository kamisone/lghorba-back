 ...
 *
 * Unlike TurnstileService (which fails open on network errors to avoid
 * blocking real users on a security gate), a failed CAPI call throws so
 * BullMQ's retry/backoff and DlqAwareWorker's dead-letter handling can take
 * over — losing an analytics event is fine to retry, never fine to silently
