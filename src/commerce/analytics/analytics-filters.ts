 ...
 *
 * Historically every endpoint took only `days` and built a rolling
 * `since = now - days*24h` window. These helpers keep that behaviour as the
 * default while adding an explicit `startDate`/`endDate` custom range, plus the
 * typed filter shapes the conversion and test-product reports accept.
 */
...
export function resolveWindow(p: DateWindowParams): DateWindow {
  ...
  if (!Number.isNaN(startMs) || !Number.isNaN(endMs)) {
    ...
    const since = !Number.isNaN(startMs)
      ? new Date(startMs)
      : new Date(until.getTime() - fallbackDays * DAY_MS);
...
export function testProductSort(raw?: string): TestProductSort | undefined {
  return TEST_PRODUCT_SORTS.includes(raw as TestProductSort)
    ? (raw as TestProductSort)
    : undefined;
}
