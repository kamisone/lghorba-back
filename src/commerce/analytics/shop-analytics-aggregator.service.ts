...
export class ShopAnalyticsAggregatorService {
  ...
  async aggregate(): Promise<void> {
    try {
      const [
        overview7d, overview30d,
        bestSellers7d, bestSellers30d,
        revenueSeries7d, revenueSeries30d,
      ] = await Promise.all([
  ...
  // ── Read from cache (falls back to live query on cache miss) ─────────────

  async getOverviewCached(days: 7 | 30): Promise<any> {
  ...
  async getBestSellersCached(days: 7 | 30, limit = 10): Promise<any> {
    const key = days === 7 ? ANALYTICS_KEYS.bestSellers7d : ANALYTICS_KEYS.bestSellers30d;
    const hit = await this.redis.client.get(key);
    if (hit) {
      const all = JSON.parse(hit) as any[];
      return all.slice(0, limit);
    }
    return this.analytics.getBestSellers(days, limit);
  }

  async getRevenueSeriesCached(days: 7 | 30): Promise<any> {
