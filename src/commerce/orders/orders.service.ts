...
export class OrdersService {
    ...
    return this.dataSource.transaction(async (em) => {
      ...
      const order = em.create(Order, {
        ...
        billingAddressSnapshot:
          (dto.billingAddress as Record<string, string>) ?? null,
        subtotalCents,
        shippingCents,
        discountCents: 0,
        taxCents: 0,
      ...
      for (const item of items) {
        await em.save(
          ...
          em.create(OrderItem, {
            ...
            compareAtPriceCentsSnapshot:
              (item as any).compareAtPriceCentsSnapshot ?? null,
      ...
      await em
        .getRepository(Cart)
        .update({ id: cart.id }, { status: 'completed' });
  ...
  async transition(
    ...
    if (!allowed.includes(toStatus)) {
      throw new BadRequestException(
        `Cannot transition from "${prev}" to "${toStatus}"`,
      );
    }
    ...
    const result = await this.dataSource.transaction(async (em) => {
      ...
      if (toStatus === 'cancelled' || toStatus === 'refunded') {
        ...
        for (const item of items) {
          ...
          if (prev === 'paid' || prev === 'processing') {
            ...
            await this.inventoryService.releaseCommittedForOrder(
              item.variantId,
              item.quantity,
              orderId,
            );
      ...
      if (toStatus === 'paid') {
        ...
        for (const item of items) {
          if (item.variantId) {
            await this.inventoryService.commitForOrder(
              item.variantId,
              item.quantity,
              orderId,
            );
      ...
      if (toStatus === 'paid') {
        await this.customerService.recordOrderCompletion(
          order.customerEmail,
          order.totalCents,
          em,
        );
  ...
  async confirmPayment(
    ...
    this.reservationQueue
      .remove(`expire-${orderId}`)
      .catch((err) =>
        this.logger.debug(
          `Could not remove expiry job for ${orderId}: ${(err as Error).message}`,
        ),
      );
    ...
    const paid = await this.transition(
      orderId,
      'paid',
    ...
    if (order.cartToken) {
      await this.cartRepo.update(
        { token: order.cartToken, status: 'active' },
        { status: 'completed' },
      );
    }
  ...
  private static readonly REVIEWABLE_STATUSES: OrderStatus[] = [
    'paid',
    'processing',
    'shipped',
    'delivered',
   ...
   * order-not-found, wrong email/token, unpaid order, and wrong product must
   * all look the same to the caller to resist enumeration.
   */
  async verifyOrderForReview(
    ...
    if (!OrdersService.REVIEWABLE_STATUSES.includes(order.status)) return null;
