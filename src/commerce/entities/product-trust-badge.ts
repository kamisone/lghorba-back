/**
 * A small icon + label "trust signal" shown in the PDP trust row
 * (e.g. "Secure checkout", "Free shipping"). Order = display order.
 * `label` holds the default-language (FR) text; FR/EN translations are
 * stored in the `translations` table under entityType `shop_product`,
 * entityId = the product's id, field = `trustBadge:{id}:label`.
 *
