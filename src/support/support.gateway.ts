...
import { SenderType } from './entities/support-message.entity';
import { WS_RATE_LIMIT_COUNT, WS_RATE_LIMIT_WINDOW, MAX_MESSAGE_LENGTH } from './support.constants';
...
function checkRate(socketId: string): boolean {
  ...
  if (!entry || now > entry.resetAt) {
    rateLimitMap.set(socketId, { count: 1, resetAt: now + WS_RATE_LIMIT_WINDOW });
    return true;
  }
...
export class SupportGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(socket: Socket): Promise<void> {
    const { guestTicket, adminToken } = socket.handshake.auth as Record<string, string>;

    // ── Admin ────────────────────────────────────────────────────────────────
    if (adminToken) {
      try {
        const payload = this.jwtService.verify<{ sub: string; email: string }>(adminToken);
  ...
  async handleAdminMessage(
    ...
    try {
      ...
      const { conversation } = await this.convService.getByIdForAdmin(data.conversationId);
  ...
  async handleConversationActive(
    ...
    const read = await this.convService.markMessagesRead(conversationId, readerType);
  ...
  emitConversationUpdate(conversationId: string, payload: Record<string, unknown>): void {
    this.server?.to('admin').emit('conversation:update', { id: conversationId, ...payload });
  }
}
