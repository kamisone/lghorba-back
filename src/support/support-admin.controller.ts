import {
  Body, Controller, Delete, Get, Param, Patch, Query, HttpCode, Req,
} from '@nestjs/common';
...
import {
  AssignDto, AssignSchema,
  StatusDto, StatusSchema,
  UpdateSettingsDto, UpdateSettingsSchema,
...
export class SupportAdminController {
  ...
  async assign(
    ...
    const conv = await this.convService.assignAdmin(id, dto.adminId, adminId(req));
    this.gateway.emitConversationUpdate(id, { assignedAdminId: conv.assignedAdminId });
    return conv;
  }
  ...
  async setStatus(
    ...
    const conv = await this.convService.setStatus(
      id,
      dto.status as ConversationStatus,
      adminId(req),
      dto.note,
    );
