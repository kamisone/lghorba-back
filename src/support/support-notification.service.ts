...
export class SupportNotificationService {
  ...
  async getSettings(): Promise<SupportNotificationSettings> {
    ...
    return {
      ...
      smsCooldownMin:     parseInt(map[SUPPORT_SETTINGS_KEYS.smsCooldownMin]         ?? String(SUPPORT_DEFAULTS.smsCooldownMin),     10),
      inactiveCloseHours: parseInt(map[SUPPORT_SETTINGS_KEYS.inactiveCloseHours]     ?? String(SUPPORT_DEFAULTS.inactiveCloseHours), 10),
    };
  }
  ...
  async updateSettings(
    ...
    if (patch.smsEnabled         !== undefined) updates.push({ key: SUPPORT_SETTINGS_KEYS.smsEnabled,         value: String(patch.smsEnabled) });
    if (patch.smsPhones          !== undefined) updates.push({ key: SUPPORT_SETTINGS_KEYS.smsPhones,          value: JSON.stringify(patch.smsPhones) });
    if (patch.smsCooldownMin     !== undefined) updates.push({ key: SUPPORT_SETTINGS_KEYS.smsCooldownMin,     value: String(patch.smsCooldownMin) });
    if (patch.inactiveCloseHours !== undefined) updates.push({ key: SUPPORT_SETTINGS_KEYS.inactiveCloseHours, value: String(patch.inactiveCloseHours) });
