...
import { DocumentService, DOCUMENTS_QUEUE, DocumentCreateJob, DocumentPdfJob, DocumentEmailJob } from './document.service';
...
export class DocumentProcessor extends DlqAwareWorker {
  ...
  async process(job: Job): Promise<void> {
    switch (job.name) {
      case 'create':     return this.handleCreate(job as Job<DocumentCreateJob>);
      case 'render-pdf': return this.handleRenderPdf(job as Job<DocumentPdfJob>);
      case 'send-email': return this.handleSendEmail(job as Job<DocumentEmailJob>);
      default: this.logger.warn(`Unknown document job: ${job.name}`);
    }
  }
  ...
  private async handleCreate(job: Job<DocumentCreateJob>): Promise<void> {
    ...
    this.logger.log(`Creating document for ${input.entityType} ${input.entityId}`);
    try {
