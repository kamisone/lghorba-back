...
export class DocumentPdfService {
  ...
  private render(doc: PDFKit.PDFDocument, document: DocumentWithLines): void {
    ...
    doc.font('Helvetica').fontSize(10).fillColor(MUTED)
       .text(`N° ${document.documentNumber ?? '—'}`, PAGE_W - MARGIN - 140, y + 6,    { width: 140, align: 'right' })
    ...
    // ── Seller / Customer / Delivery columns ──────────────────────────────────
    const colW = (COL_W - 20) / 2;
    const cx   = MARGIN + colW + 20;

    doc.font('Helvetica-Bold').fontSize(8).fillColor(MUTED)
       .text('ÉMETTEUR', MARGIN, y)
       .text(isInvoice ? 'CLIENT' : 'LIVRAISON', cx, y);
    y += 14;
    ...
    let sy = y + 14;
    if (addr.line1)   { doc.text(addr.line1,   MARGIN, sy, { width: colW }); sy += 12; }
    const cz = [addr.zip, addr.city].filter(Boolean).join(' ');
    if (cz)           { doc.text(cz,            MARGIN, sy, { width: colW }); sy += 12; }
    if (addr.country) { doc.text(addr.country,  MARGIN, sy, { width: colW }); sy += 12; }
    if (document.sellerVatNumber) { doc.text(`TVA : ${document.sellerVatNumber}`, MARGIN, sy, { width: colW }); sy += 12; }
    if (document.sellerSiret)     { doc.text(`SIRET : ${document.sellerSiret}`,   MARGIN, sy, { width: colW }); }
    ...
    doc.font('Helvetica').fontSize(9).fillColor(SLATE);
    let cy2 = y + 14;
    doc.text(document.customerEmail, cx, cy2, { width: colW }); cy2 += 12;
    if (addressBlock?.line1)   { doc.text(addressBlock.line1,   cx, cy2, { width: colW }); cy2 += 12; }
    const az = [addressBlock?.zip, addressBlock?.city].filter(Boolean).join(' ');
    if (az) { doc.text(az, cx, cy2, { width: colW }); cy2 += 12; }
    if (addressBlock?.country) { doc.text(addressBlock.country, cx, cy2, { width: colW }); }
    ...
    // ── Reference line ────────────────────────────────────────────────────────
    const refLabel  = isInvoice ? 'Réservation' : 'Commande';
    doc.font('Helvetica').fontSize(9).fillColor(MUTED)
       .text(`${refLabel} : `, MARGIN, y, { continued: true })
       .font('Helvetica-Bold').fillColor(DARK).text(document.entityId);
    y += 24;

    // ── Line items table ──────────────────────────────────────────────────────
    const C = {
      desc:  { x: MARGIN,            w: COL_W * 0.50 },
      sku:   { x: MARGIN + COL_W * 0.50, w: COL_W * 0.12 },
    ...
    doc.text('DÉSIGNATION',  C.desc.x  + 4, y + 6, { width: C.desc.w  - 8 });
    doc.text('SKU',          C.sku.x   + 4, y + 6, { width: C.sku.w   - 8 });
    doc.text('QTÉ',          C.qty.x   + 4, y + 6, { width: C.qty.w   - 8, align: 'center' });
    doc.text('P.U. TTC',     C.unit.x  + 4, y + 6, { width: C.unit.w  - 8, align: 'right' });
    ...
    tot('Sous-total HT', document.subtotalCents - document.taxCents);
    if (document.deliveryCents) tot('Livraison / Livraison',    document.deliveryCents);
    if (document.discountCents) tot(`Remise${document.couponCode ? ` (${document.couponCode})` : ''}`, -document.discountCents);
    tot(`${document.taxLabel ?? 'TVA'} (${document.taxRatePct}%)`, document.taxCents);

    y += 2;
    doc.rect(totX, y, totW, 24).fill(bgColor);
    doc.font('Helvetica-Bold').fontSize(11).fillColor(fgColor)
    ...
    doc.font('Helvetica-Oblique').fontSize(8).fillColor(MUTED)
       .text('Paiement reçu par carte bancaire via Stripe. Ce document est acquitté.', MARGIN, y);
    y += 24;
    ...
    const footer = [
      document.sellerName,
      document.sellerVatNumber ? `TVA ${document.sellerVatNumber}` : null,
      document.sellerSiret     ? `SIRET ${document.sellerSiret}`   : null,
    ].filter(Boolean).join(' · ');
    doc.font('Helvetica').fontSize(7).fillColor(LIGHT)
       .text(footer, MARGIN, y, { width: COL_W, align: 'center' });
  }
}
